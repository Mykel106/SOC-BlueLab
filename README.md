...
- I believe I have setup a "double Nat" which is causing a mismatched IP source IP in the logs. Kali Laptop scan showed up in Splunk with the Source IP of 192.168.10.100 instead of the  Kali's actual IP of 10.0.0.2. I traced it back to a Netgear Router I do have setup as a WiFi Extender, but it is running in full router mode instead of an AP. This shows me that it is running its own private network  and uses NAT to rewrite the Kali's real IP with it own address before the traffic ever reached the rest of the network. LESSON: If NAT sits anywhere in the path, the source IP in a log may belong to the NAT device not the actual machine. 
